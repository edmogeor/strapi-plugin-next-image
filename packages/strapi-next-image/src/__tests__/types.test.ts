...
describe('isStrapiMedia()', () => {
  it('returns true for valid StrapiMedia', () => {
    expect(isStrapiMedia({ id: 1, url: '/photo.jpg', width: 800, height: 600 })).toBe(true);
  });
  ...
  it('returns false when width is missing', () => {
    expect(isStrapiMedia({ id: 1, url: '/photo.jpg', height: 600 })).toBe(false);
  });

  it('returns false when height is missing', () => {
    expect(isStrapiMedia({ id: 1, url: '/photo.jpg', width: 800 })).toBe(false);
  });

  it('returns false when url is not a string', () => {
    expect(isStrapiMedia({ id: 1, url: 123, width: 800, height: 600 })).toBe(false);
  });
});
